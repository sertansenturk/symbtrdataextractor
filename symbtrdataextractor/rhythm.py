...
def extract_rhythmic_structure(score):
        ...
        if usul['mu2_name'] == '[Serbest]':
            ...
            while not tempo:
                ...
                if score['code'][it] == 9:  # proper note
                    tempo = compute_tempo_from_note(score['numerator'][it],
                                                    score['denumerator'][it],
                                                    score['duration'][it],
                                                    usul['mertebe'])

        rhythmic_structure.append({'usul': usul,
                                   'tempo': {'value': tempo, 'unit': 'bpm'},
                                   'startNote': start, 'endNote': end})

    return rhythmic_structure


def compute_tempo_from_note(note_num, note_denum, note_dur, mertebe):
