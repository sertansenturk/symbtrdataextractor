...
def get_slug(scorename):
    splitted = scorename.split('--')
    return {'makam': splitted[0], 'form': splitted[1], 'usul': splitted[2],
            'name': splitted[3], 'composer': splitted[4]}


def get_metadata(scorename, mbid='', get_recording_rels=False):
    if mbid:
        data = get_metadata_from_muscbrainz(
            mbid, get_recording_rels=get_recording_rels)
    else:
    ...
    makam = get_makam(data['makam']['symbtr_slug'])
    is_makam_valid = validate_attribute(data['makam'], makam, scorename)

    form = get_form(data['form']['symbtr_slug'])
    is_form_valid = validate_attribute(data['form'], form, scorename)

    usul = get_usul(data['usul']['symbtr_slug'])
    is_usul_valid = validate_attribute(data['usul'], usul, scorename)
...
def get_attribute_key(attr_str, attr_type):
    attr_dict = get_attribute_dict(attr_type)
...
def validate_attribute(score_attrib, attrib_dict, scorename):
            ...
            for uv in attrib_dict['variants']:
                if uv['mu2_name'] == score_attrib['mu2_name']:
                    ...
                    if not uv['num_pulses'] == \
                        ...
                        is_attribute_valid = False
                        print("    " + scorename + ', ' + uv['mu2_name'] +
                              ': The number of pulses in an usul cycle does '
                              'not match.')
            if not mu2_name:  # no matching variant
                is_attribute_valid = False
                print("    " + scorename + ', ' + score_attrib['mu2_name'] +
    ...
    if 'mb_attribute' in score_attrib.keys():  # work
        ...
        if score_attrib['symbtr_slug'] in skip_makam_slug:
            print("    " + scorename + ': The usul attribute is not stored '
                                       'in MusicBrainz.')
        else:
            if not score_attrib['mb_attribute'] == attrib_dict['dunya_name']:
                # dunya_names are (or should be) a superset of the musicbrainz
                # attributes
                is_attribute_valid = False
                if score_attrib['mb_attribute']:
                    ...
                    print("    " + scorename + ': The MusicBrainz attribute '
                                               'does not exist.')
...
def get_makam(makam_slug):
    makam_dict = get_attribute_dict('makam')
...
def get_form(form_slug):
    form_dict = get_attribute_dict('form')
...
def get_metadata_from_muscbrainz(mbid, get_recording_rels=False):
    o = urlparse(mbid)
    if o.netloc:  # url supplied
        o_splitted = o.path.split('/')
        if o_splitted[1] == 'work':
            data = get_work_metadata_from_musicbrainz(o_splitted[2],
            ...
            data = get_recording_metadata_from_musicbrainz(o_splitted[1])
    else:  # mbid supplied
        try:  # assume mbid is a work
            data = get_work_metadata_from_musicbrainz(mbid,
                                                      get_recording_rels=False)
        except:  # assume mbid is a recording
            if get_recording_rels:
                print("    " + "Recording mbid is given. Ignoring "
                               "get_recording_rels input")
            data = get_recording_metadata_from_musicbrainz(mbid)
    return data


def get_work_metadata_from_musicbrainz(mbid, get_recording_rels=False):
...
def get_recording_metadata_from_musicbrainz(mbid):
    rec = mb.get_recording_by_id(mbid,
                                 includes=['artist-rels', 'tags'])['recording']

    data = ({'makam': [], 'form': [], 'usul': [], 'recording': {'mbid': mbid},
             'performers': []})
    ...
    data['makam'] = data['makam'][0] if len(data['makam']) == 1 else \
        data['makam']
