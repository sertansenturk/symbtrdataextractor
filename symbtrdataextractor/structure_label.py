...
def label_structures(structures, score, lyrics_sim_thres, melody_sim_thres):
    ...
    for s in structures:
        ...
        score_fragments.append({'durs': durs, 'nums': nums, 'denums': denums,
                                'notes': notes, 'lyrics': lyrics})

    # get the lyric organization
    structures = get_lyric_organization(structures, score_fragments,
                                        lyrics_sim_thres)

    # get the melodic organization
    structures = get_melodic_organization(structures, score_fragments,
                                          melody_sim_thres)

    return structures


def get_lyric_organization(structures, score_fragments, lyrics_sim_thres):
    ...
    if structures:
        ...
        dists = matrix([[normalized_levenshtein(a['lyrics'], b['lyrics'])
                         for a in score_fragments] for b in score_fragments])

        cliques = get_cliques(dists, lyrics_sim_thres)
...
def get_melodic_organization(structures, score_fragments, melody_sim_thres):
    if structures:
        ...
        cliques = get_cliques(dists, melody_sim_thres)
        ...
        for i in range(0, len(melody_labels)):
            if all(lbl not in structures[i]['name']
                ...
                mel_str = (structures[i]['slug'] + '_' + melody_labels[i][1:]
...
def semiotize(cliques):
    ...
    sim_clq_it = [1] * len(cliques['similar'])  # idx to label similar cliques
