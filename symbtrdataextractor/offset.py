def find_measure_start_idx(offsets, print_warnings=True):
    measure_start_idx = []
    ...
    non_integer_measure_start = ([offsets[i] for i in measure_start_idx
                                  if not is_integer_offset(offsets[i])])
    if non_integer_measure_start and print_warnings:
